...
for indie_tool in cmake curl make patch shasum tar xcrun; do
...
indie_llvm_clang="/opt/homebrew/opt/llvm/bin/clang"
indie_lld="/opt/homebrew/opt/lld/bin/lld-link"
if [[ ! -x "$indie_bison" || ! -x "$indie_flex" || ! -x "$indie_llvm_clang" || ! -x "$indie_lld" ]]; then
  print -u2 "需要 Homebrew bison、flex、LLVM 与 LLD：brew install bison flex llvm lld"
    exit 2
fi
...
arch -x86_64 /usr/bin/env \
  ...
  LDFLAGS="-arch x86_64 -L$indie_dependency_root/lib" \
...
arch -x86_64 /usr/bin/env \
  ...
  "$indie_wine_source/configure" \
    ...
    --with-mingw="$indie_llvm_clang" \
    --without-x --without-wayland --without-vulkan --without-gstreamer \
...
find "$indie_stage/lib/wine/i386-windows" "$indie_stage/lib/wine/x86_64-windows" -type f -print0 | while IFS= read -r -d '' indie_pe; do
  if file "$indie_pe" | grep -q "PE32"; then
    /opt/homebrew/opt/llvm/bin/llvm-strip --strip-all "$indie_pe"
  fi
done
...
for indie_library in \
  ...
  libhogweed.6.9.dylib libgnutls.30.dylib; do
