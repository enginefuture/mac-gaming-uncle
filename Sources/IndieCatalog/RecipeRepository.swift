...
public struct RecipeRepository: Sendable {
    ...
    public static func load(from directory: URL) throws -> RecipeValidation {
        guard let enumerator = FileManager.default.enumerator(at: directory, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles]) else {
            throw IndieError.notFound("无法读取配方目录：\(directory.path)")
        }
        ...
        for case let url as URL in enumerator where url.pathExtension.lowercased() == "json" {
            do {
                ...
                guard recipe.schemaVersion == 1 else { throw IndieError.invalidData("未知 schemaVersion") }
                guard !recipe.id.isEmpty, !recipe.name.isEmpty, !recipe.profiles.isEmpty else { throw IndieError.invalidData("缺少 id、name 或 profiles") }
                guard ids.insert(recipe.id).inserted else { throw IndieError.invalidData("重复配方 id：\(recipe.id)") }
